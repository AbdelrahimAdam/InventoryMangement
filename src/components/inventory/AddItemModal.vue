...
export default {
  ...
  try {
    ...
    // CRITICAL FIX: Emit the result to parent (Inventory component)
    console.log('📤 Emitting result to parent...');
    emit('success', result);
    ...
    // Update local store - REMOVE THIS LINE (let parent handle it)
    // if (result.item && result.item.id) {
    //   store.commit('UPDATE_INVENTORY_ITEM', result.item);
    //   console.log('🔄 Item updated in local store');
    // }

    // Silent background refresh - REMOVE THIS (let parent handle it)
    // setTimeout(async () => {
    //   try {
