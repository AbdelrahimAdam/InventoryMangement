...
// Core Firebase imports (loaded immediately)
const firebaseCore = {
  auth
};
...
let inventoryServiceModule = null;
let userServiceModule = null;

// Function to lazy load Firestore
...
// Function to lazy load InventoryService
const loadInventoryService = async () => {
  if (!inventoryServiceModule) {
    inventoryServiceModule = await import('@/services/inventoryService');
  }
  return inventoryServiceModule;
};

// Function to lazy load UserService
const loadUserService = async () => {
  if (!userServiceModule) {
    userServiceModule = await import('@/services/UserService');
  }
  return userServiceModule;
};

// Field name mapping (static, loaded immediately)
const FIELD_MAPPINGS = {
  ...
  englishToArabic: {
    ...
    'per_carton_count': 'عدد في الكرتونه',
...
const mutations = {
  ...
  INVALIDATE_CACHE(state, cacheKey) {
...
const actions = {
  ...
  async loadUserProfile({ commit, dispatch }, user) {
    try {
      ...
      const userDoc = await getDoc(doc(firestore.db, 'users', user.uid));
      ...
      if (!userDoc.exists()) {
        ...
        await setDoc(doc(firestore.db, 'users', user.uid), tempProfile);
  ...
  async notifyAdminAboutPendingUser({ dispatch }, { userId, userEmail }) {
    try {
      const firestore = await loadFirestore();
      const { doc, collection, setDoc } = firestore;

      const notificationRef = doc(collection(firestore.db, 'admin_notifications'));
  ...
  async login({ commit, dispatch }, { email, password }) {
    ...
    try {
      ...
      const userCredential = await signInWithEmailAndPassword(firebaseCore.auth, email, password);
  ...
  async logout({ commit, dispatch }) {
    try {
      await signOut(firebaseCore.auth);
      commit('SET_USER', null);
      commit('SET_USER_PROFILE', null);
      commit('SET_INVENTORY', []);
      commit('SET_TRANSACTIONS', []);
  ...
  async createWarehouse({ commit, dispatch, state }, warehouseData) {
    ...
    try {
      ...
      const warehouseRef = await addDoc(collection(firestore.db, 'warehouses'), newWarehouse);

      commit('INVALIDATE_CACHE', 'warehouses');
  ...
  async updateWarehouse({ commit, dispatch, state }, { warehouseId, warehouseData }) {
    ...
    try {
      ...
      const warehouseRef = doc(firestore.db, 'warehouses', warehouseId);
  ...
  async deleteWarehouse({ commit, dispatch, state }, warehouseId) {
    ...
    try {
      ...
      const firestore = await loadFirestore();
      const { doc, getDoc, collection, query, where, limit, getDocs, deleteDoc } = firestore;
  ...
  async updateItem({ commit, dispatch, state }, { itemId, itemData }) {
    ...
    try {
      ...
      const firestore = await loadFirestore();
      const { doc, getDoc, updateDoc, collection, addDoc } = firestore;

      const itemRef = doc(firestore.db, 'items', itemId);
      ...
      const newTotalQuantity = inventoryService.InventoryService.calculateTotalQuantity(
        newCartonsCount,
        newPerCartonCount,
      ...
      if (quantityDiff !== 0 || existingItem.warehouse_id !== warehouse_id) {
        ...
        await addDoc(collection(firestore.db, 'transactions'), transactionData);
      }

      commit('INVALIDATE_CACHE', 'inventory');
  ...
  async deleteItem({ commit, dispatch, state }, itemId) {
    ...
    try {
      ...
      const firestore = await loadFirestore();
      const { doc, getDoc, deleteDoc, collection, addDoc } = firestore;
  ...
  async loadAllUsers({ commit, dispatch, getters }) {
    ...
    try {
      ...
      const result = await userService.default.getUsers();
  ...
  async createUser({ commit, dispatch, state }, userData) {
    commit('SET_OPERATION_LOADING', true);
    commit('CLEAR_OPERATION_ERROR');
    ...
    try {
      ...
      const userService = await loadUserService();
      const result = await userService.default.createUser(userData, superadminCredentials);
  ...
  async updateUser({ commit, dispatch, getters }, { userId, userData }) {
    ...
    try {
      ...
      const userService = await loadUserService();
      const result = await userService.default.updateUser(userId, userData);
  ...
  async updateUserStatus({ commit, dispatch, getters }, { userId, isActive }) {
    ...
    try {
      ...
      const userService = await loadUserService();
      const result = await userService.default.updateUserStatus(userId, isActive);
  ...
  async getUserStats({ commit, dispatch, getters }) {
    try {
      ...
      const userService = await loadUserService();
      const result = await userService.default.getUserStats();
  ...
  async checkExistingItem({ state }, itemData) {
    try {
      ...
      const itemsRef = collection(firestore.db, 'items');
      const q = query(
        itemsRef,
  ...
  async addInventoryItemAtomic({ commit, dispatch, state }, { itemData, userId, isAddingCartons = true }) {
    ...
    try {
      ...
      const inventoryService = await loadInventoryService();
      const totalQuantity = inventoryService.InventoryService.calculateTotalQuantity(
      ...
      const firestore = await loadFirestore();
      const { doc, updateDoc, addDoc, collection } = firestore;

      let result;
      if (existingItem) {
        const itemRef = doc(firestore.db, 'items', existingItem.id);
        ...
        const docRef = await addDoc(collection(firestore.db, 'items'), newItemData);
  ...
  async loadWarehouses({ commit, dispatch, state }) {
    try {
      // Check cache first
      const cache = state.cache.warehouses;
      if (cache?.data && Date.now() - cache.timestamp < cache.ttl) {
        console.log('Loading warehouses from cache');
        commit('SET_WAREHOUSES', cache.data);
  ...
  async subscribeToWarehouses({ commit, state, dispatch }) {
    ...
    try {
      ...
      const warehousesQuery = query(
        collection(firestore.db, 'warehouses'),
        orderBy('name_ar')
      );
  ...
  async initializeDefaultWarehouses({ commit, dispatch }) {
    try {
      ...
      const firestore = await loadFirestore();
      const { doc, getDoc, writeBatch } = firestore;
      ...
      for (const warehouse of defaultWarehouses) {
        const warehouseRef = doc(firestore.db, 'warehouses', warehouse.id);
        const warehouseDoc = await getDoc(warehouseRef);
    ...
    try {
      ...
      const unsubscribe = onSnapshot(itemsQuery, (snapshot) => {
        ...
        const inventory = snapshot.docs.map(doc => {
          const data = doc.data();
          return inventoryService.InventoryService.convertForDisplay({
            id: doc.id,
            ...data
          });
        });
  ...
  async addInventoryItem({ commit, dispatch, state }, { itemData, isAddingCartons = true }) {
    ...
    try {
      ...
      const result = await inventoryService.InventoryService.addOrUpdateItem(
        cleanedData,
        state.user.uid,
        isAddingCartons
      );
  ...
  async transferItem({ commit, dispatch, state }, transferData) {
    ...
    try {
      ...
      const inventoryService = await loadInventoryService();
      const result = await inventoryService.InventoryService.transferItem(transferData, state.user.uid);
  ...
  async dispatchItem({ commit, dispatch, state }, dispatchData) {
    ...
    try {
      ...
      const inventoryService = await loadInventoryService();
      const result = await inventoryService.InventoryService.dispatchItem(dispatchData, state.user.uid);
